...
const isDockerUri = RegExp.prototype.test.bind(
      ...
      serverless.configSchemaHandler.defineProvider('aws', {
                ...
                '^[a-z]{1,64}$': {
                  oneOf: [
                    // TODO: Combine into one?
                    {
                      type: 'object',
                      properties: {
                        uri: { $ref: '#/definitions/dockerImageUri' },
                      },
                      required: ['uri'],
                      additionalProperties: false,
                    },
        ...
        function: {
          properties: {
            ...
            image: {
              oneOf: [
                { $ref: '#/definitions/dockerImageUri' },
                {
                  type: 'string',
                  pattern: '^[a-z]{1,64}$',
                },
  ...
  memoizeeMethods({
    dockerLoginToEcr: d(
      ...
      async function () {
        ...
        // TODO: Better information about how credentials are stored?
      },
      { promise: true }
    ),
    getOrCreateEcrRepository: d(
      async function () {
        const repositoryName = this.getEcrRepositoryName();
        let repositoryUri;
        try {
    ...
    resolveImageUriAndShaFromPath: d(
      // TODO: TESTS
      // TODO: Check for error from spawn calls?
      async function (imageName, imagePath) {
        ...
        const buildDockerArgs = ['build', '-t', `${repositoryName}:${imageName}`, imagePath];
        await spawnExt('docker', buildDockerArgs, { stdio: 'inherit' });

        const tagDockerArgs = [
          'tag',
          `${repositoryName}:${imageName}`,
          `${repositoryUri}:${imageName}`,
        ];
        await spawnExt('docker', tagDockerArgs, { stdio: 'inherit' });

        const pushDockerArgs = ['push', `${repositoryUri}:${imageName}`];
        const dockerPushPromise = spawnExt('docker', pushDockerArgs);
        dockerPushPromise.stdout.pipe(process.stdout);
        dockerPushPromise.stderr.pipe(process.stderr);
        await dockerPushPromise;

        // Extract imageSha from `docker push` output
        const imageSha = dockerPushPromise.stdoutBuffer
          .toString()
    ...
    resolveImageUriAndSha: d(
      ...
      async function (image) {
        ...
        const imageDefinedInProvider = this.serverless.service.provider.images[image];
        if (imageDefinedInProvider) {
          ...
          if (isDockerUri(imageDefinedInProvider)) {
